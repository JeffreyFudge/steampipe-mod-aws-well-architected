...
# Modified by jfudge to eliminate all positive findings (due to potential of many backups) - added where clause
...
# Modified by jfudge to eliminate all positive findings (potential of many backups) - added where clause
query "backup_plan_min_retention_35_days" {
  sql = <<-EOQ
    ...
    from
      all_plans as r
      where r.Rules is null;
  EOQ
}

# Modified by jfudge to eliminate positive findings due to potential numbmer of backups (added where clause)
...
query "backup_recovery_point_min_retention_35_days" {
  sql = <<-EOQ
    ...
    from
      aws_backup_recovery_point
    where lifecycle -> 'DeleteAfterDays' is not null;
  EOQ
}
