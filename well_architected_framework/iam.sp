...
control "iam_policy_no_star_star" {
  ...
  description = "AWS Identity and Access Management (IAM) can help you incorporate the principles of least privilege and separation of duties with access permissions and authorizations, restricting policies from containing 'Effect': 'Allow' with 'Action': '*' over 'Resource': '*'.  All positive findings are hidden due to reduce report size."
...
control "iam_all_policy_no_service_wild_card" {
  ...
  description = "Checks if AWS Identity and Access Management (IAM) policies grant permissions to all actions on individual AWS resources. The rule is non-compliant if the managed IAM policy allows full access to at least 1 AWS service. All positive findings are hidden to reduce report size."
...
control "iam_policy_custom_no_blocked_kms_actions" {
  ...
  description = "Checks if the managed AWS Identity and Access Management (IAM) policies that you create do not allow blocked actions on AWS KMS keys. The rule is non - compliant if any blocked action is allowed on AWS KMS keys by the managed IAM policy. All positive findings are hidden to reduce report size."
...
control "iam_managed_policy_attached_to_role" {
  ...
  description = "This control checks if all AWS managed policies specified in the list of managed policies are attached to the AWS Identity and Access Management (IAM) role. The rule is non-compliant if an AWS managed policy is not attached to the IAM role. All positive findings are hidden to reduce report size."
...
control "iam_policy_no_full_access_to_kms" {
  ...
  description = "KMS is a critical service and IAM policies should follow least privilege model for this service in particular. This control is non-compliant if the managed IAM policy allows full access to KMS service. All positive findings are hidden to reduce report size."
  query       = query.iam_policy_no_full_access_to_cloudtrail
...
# modified by jfudge to eliminate postive findings (to reduce report size) - added where clause
...
# modified by jfudge to eliminate positive findings (to reduce report size) - added where clause
...
# modified by jfudge due to potential of many positive findings (added where clause)
