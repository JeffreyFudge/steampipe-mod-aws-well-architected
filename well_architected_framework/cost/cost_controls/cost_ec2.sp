...
locals {
  ec2_common_tags = merge(local.well_architected_framework_cost_common_tags, {
    service = "AWS/EC2"
  })
}

benchmark "cost_ec2" {
  title         = "Cost Optimization: EC2"
  description   = "Unused or underutilized EC2 resources should be regularly reviewed."
  children = [
    control.cost_ec2_application_lb_unused,
    control.cost_ec2_classic_lb_unused,
    control.cost_ec2_gateway_lb_unused,
    control.cost_ec2_instance_older_generation,
    control.cost_ec2_instance_with_graviton,
    control.cost_ec2_network_lb_unused,
    control.cost_ec2_reserved_instance_lease_expiration_days,
    control.cost_instances_with_low_utilization,
    control.cost_large_ec2_instances,
    control.cost_long_running_ec2_instances
  ]
  ...
  sql = <<-EOQ
    ...
    select
      ...
      case
        ...
        when avg_max < 15 then 'alarm'
        when avg_max < 30 then 'info'
        else 'ok'
      end as status,
      case
        when avg_max is null then 'CloudWatch metrics not available for ' || title || '.'
        else title || ' is averaging ' || round(avg_max::decimal,2) || '% max utilization over the last ' || days || ' days.'
      end as reason
    from
      aws_ec2_instance i
