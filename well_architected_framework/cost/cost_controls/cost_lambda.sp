locals {
  lambda_common_tags = merge(local.well_architected_framework_cost_common_tags, {
    service = "AWS/Lambda"
  })
}

benchmark "lambda" {
  title         = "Cost Optimization: Lambda"
  description   = "Ensure Lambda functions are cost optimized."
  ...
  sql = <<-EOQ
    ...
    select
      ...
      case
        ...
        else title || ' error rate is ' || round(error_rate::decimal,2) || '% the last ' || '7  days.'
      end as reason
    from
