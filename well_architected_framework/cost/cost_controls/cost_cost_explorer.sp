...
locals {
  cost_explorer_common_tags = merge(local.well_architected_framework_cost_common_tags, {
    service = "AWS/CostExplorer"
  })
}

benchmark "cost_explorer" {
  title         = "Cost Optimization: Cost Explorer"
  description   = "Routinely monitor cloud usage and cost data."
...
control "full_month_cost_changes" {
  ...
  description = "This compares the cost of services between the last two full months of AWS usage."
  severity    = "low"
