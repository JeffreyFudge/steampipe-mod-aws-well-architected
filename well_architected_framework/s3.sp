...
control "s3_bucket_restrict_public_read_access" {
  ...
  description = "Manage access to resources in the AWS Cloud by only allowing authorized users, processes, and devices access to AWS Simple Storage Service (AWS S3) buckets.  All positive findings are hidden due to reduce report size."
...
control "s3_bucket_policy_restrict_public_access" {
  ...
  description = "This control checks that the access granted by the S3 bucket is restricted by any of the principals, federated users, service principals, IP addresses, or VPCs that you provide. The rule is compliant if a bucket policy is not present.  All positive findings hidden to reduce report size."
...
query "s3_bucket_restrict_public_read_access" {
  sql = <<-EOQ
    ...
    where (block_public_acls or a.name is null) and (bucket_policy_is_public and not block_public_policy)  
    or (block_public_acls or a.name is null) and (bucket_policy_is_public and p.name is not null);
  EOQ
}
